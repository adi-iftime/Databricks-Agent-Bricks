...
Stub jobs use **serverless compute**: notebook tasks only, with no `job_clusters` / `new_cluster` (required for workspaces that only allow serverless).
...
Classic cluster configs and governance tags can return in a follow-up when a classic-capable workspace or approved serverless environments are available.
